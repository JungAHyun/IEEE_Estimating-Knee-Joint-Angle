...
num = 3
while(num<13):
    input_filepath_Left = 'C:\\Users\\JAH\\Desktop\\논문\\f-scan laft data\\L_data'+ str(num) +'.csv'        #족저압 input데이터  파일경로
    left_input_df = pd.read_csv(input_filepath_Left,encoding='cp949')                                                       #input 데이터 csv파일 불러와서 데이터프레임 저장    

    input_filepath_Right = 'C:\\Users\\JAH\\Desktop\\논문\\f-scan right data\\R_data'+str(num)+'.csv'         #족저압 input데이터  파일경로
    right_input_df = pd.read_csv(input_filepath_Right,encoding='cp949')                                                       #input 데이터 csv파일 불러와서 데이터프레임 저장 

    angle_filepath = 'C:\\Users\\JAH\\Desktop\\논문\\azure_data\\angle'+str(num)+'.csv'
    angle_df = pd.read_csv(angle_filepath,encoding='cp949') 

    save_filepath = 'C:\\Users\\JAH\\Desktop\\논문\\input_data\\pp_to_ja'+str(num)+'.csv'
    fdata_check_filepath= 'C:\\Users\\JAH\\Desktop\\논문\\input_data\\data_check'+str(num)+'.csv'
...
class SaveCSV:
    ...
    def __init__(self) :
        self.save_filepath = 'DPP_Data\\pp_to_ja12.csv'
        self.f =  open(self.save_filepath, 'w', encoding='utf-8', newline="")

        self.fdata_check_filepath= 'DPP_Data\\data_check12.csv'
    ...
    def save_csv(self, start_index, ja_data, pp_left_data, pp_right_data, pp_left_rawsum, pp_right_rawsum, all_angle):
        ...
        if(pp_right_rawsum < 100): 
            self.write_data.append('Left')

        else:
            self.write_data.append('Right')

        for i in range(59):
            if(pp_left_rawsum < 100):
                self.write_data.append(pp_right_data[i])
            else:
                self.write_data.append(pp_left_data[i])
...
def angle_sheet(all_angle):
    interpolation_angle = 'DPP_Data\\interpolation_angle12.csv'
