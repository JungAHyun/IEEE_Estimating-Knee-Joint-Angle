...
class DataReader():
    def __init__(self):
        self.x_train_filepath = 'Dataset\\no shuffle_side_both_x_train_dataset.csv'
        self.y_train_filepath = 'Dataset\\no shuffle_side_both_y_train_dataset.csv'
        self.x_test_filepath = 'Dataset\\no shuffle_side_both_x_test_dataset.csv'
        self.y_test_filepath = 'Dataset\\no shuffle_side_both_y_test_dataset.csv'
...
if __name__ == '__main__':
    ...
    x_test = x_test.reshape(-1,59,42,1)
    ...
    hist_decay = model.fit(x_train, y_train, epochs=100, shuffle=True, batch_size = 256)
    ...
    save_filepath = 'Result\\no_shuffle_both_result.csv'
    f =  open(save_filepath, 'w', encoding='utf-8', newline="")
    wr = csv.writer(f)
    wr.writerow(["Joint_Angle", "Predict_Angle", "Plantar_Pressure"])
    for i in range(len(y_test)-1):
        ...
        for j in range(len(x_test[i])):
            ...
            x_data.append(tmp)
        wr.writerow([y_test[i], y_pred_list[i], x_data])


    print("========= Result =========")
    print("MAE: ",mean_absolute_error(y_true=y_test, y_pred=y_pred_list))
    print("MAE2: ", np.mean(np.abs(y_test - y_pred_list)))
    print("std: ", np.std(np.absolute(np.subtract(y_test, y_pred_list))))
    print("=========================")
    print("ME: ", np.mean(np.subtract(y_test, y_pred_list)))
    print("std: ", np.std(np.subtract(y_test, y_pred_list)))
    print("=========================")
    print("mean relative error: ", np.mean(np.divide(np.absolute(np.subtract(y_test, y_pred_list)), y_test))*100)
    print("=========================")        
    print("r2 score: ", r2_score(y_true=y_test, y_pred=y_pred_list))
    print("=========================")
